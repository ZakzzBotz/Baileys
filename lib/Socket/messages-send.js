...
const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, cachedGroupMetadata, useCachedGroupMetadata, statusJidList, AI = true }) => {
        ...
        const destinationJid = (!isStatus)
            ? WABinary_1.jidEncode(user, isLid ? 'lid' : isGroup ? 'g.us' : isNewsletter ? 'newsletter' : 's.whatsapp.net')
            : statusJid;
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroup || isStatus) {
                ...
                const patched = await patchMessageBeforeSending(
                    message,
                    devices.map(d => WABinary_1.jidEncode(d.user, isLid ? 'lid' : 's.whatsapp.net', d.device))
                );
    ...
    const getMediaType = (message) => {
        ...
        else if (/https:\/\/wa\.me\/p\/\d+\/\d+/.test(message.extendedTextMessage?.text)) return 'productlink';
    };
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            // toggle disappearing messages (group only)
